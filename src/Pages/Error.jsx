...
// The svg used to render at a fixed 230 by 270 pixels regardless of screen
// size. That happened to fit on virtually every phone by coincidence, since
// 230px is smaller than almost any real device width, but it's the same
// pattern that broke other pages when their fixed sizes were bigger. Wrapping
// it in a container with a max width and an aspect ratio means it now
// scales down cleanly on anything narrower too, a folded phone, an embedded
// webview, whatever comes along, instead of relying on luck.
function ErrorCubeCard({image}) {
    return (
        <div className="w-full max-w-[230px] aspect-[230/270] mx-auto">
            <svg
                width="100%"
                height="100%"
...
function Error() {
    return (
        <div className="relative min-h-screen overflow-hidden bg-[#0d1117]">
            ...
            {/* Content */}
            <div
                className="relative z-10 min-h-screen flex flex-col items-center justify-center text-center px-4 gap-4 sm:gap-6">

                <ErrorCubeCard image={ERROR_IMAGE}/>

                <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold text-[#e2f0ee]">
                    404
                </h1>

                <p className="text-base sm:text-lg text-[#e2f0ee]/70">
